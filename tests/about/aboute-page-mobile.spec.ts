...
test.describe('About page', { tag: ['@UI', '@Mobile'] }, () => {
  ...
  test.describe('Navbar validation', { tag: ['@Smoke'] }, () => {
    ...
    test('Menu button is displayed', async ({ aboutPage }) => {
      await expect(aboutPage.$mobileNavBar).toBeVisible();
    });

    const menuOptions: NavButton[] = ['About', 'Docs', 'Swagger'];
    test(`Menu options are [${menuOptions}]`, async ({ aboutPage }) => {
      await aboutPage.$mobileNavBar.click();
      await expect(aboutPage.$mobileNavMenuItems).toHaveText(menuOptions);
    });
